...
# GYG often appends managed_by=... after login; keep it so pagination URLs stay valid.
_REVIEWS_MANAGED_BY = (os.getenv("GYG_MANAGED_BY") or "").strip() or None

def extract_query_param(url, name):
    try:
        values = parse_qs(urlparse(str(url)).query).get(name) or []
        return values[0] if values else None
    except Exception:
        return None

def remember_managed_by_from_url(url):
    global _REVIEWS_MANAGED_BY
    managed = extract_query_param(url, "managed_by")
    if managed:
        _REVIEWS_MANAGED_BY = managed

def get_reviews_page_index(url):
    """Returns the 0-based page index from a GYG reviews URL (missing page => 0)."""
    raw = extract_query_param(url, "page")
...
def is_reviews_list_url(url):
    try:
        path = urlparse(str(url)).path.rstrip("/").lower()
        return path.endswith("/performance/reviews")
    except Exception:
        return False

def reviews_url_matches_page(url, page_index):
    """True when URL is the reviews list on the expected page, ignoring extra params like managed_by."""
    if not is_reviews_list_url(url):
        return False
    return get_reviews_page_index(url) == max(0, int(page_index))

def build_reviews_url(page_index, managed_by=None):
    managed = managed_by or _REVIEWS_MANAGED_BY
    base = "https://supplier.getyourguide.com/performance/reviews"
    params = {}
    if page_index > 0:
        params["page"] = str(int(page_index))
    if managed:
        params["managed_by"] = str(managed)
    if not params:
        return base
...
def scrape_cycle_in_session(browser, context, page, owns_browser, recreate_fresh_context, is_headless):
    ...
    try:
        ...
        remember_managed_by_from_url(page.url)
        ...
        for page_num in range(MAX_PAGES):
            ...
            remember_managed_by_from_url(page.url)
            previous_page_index = page_num
            navigated_via_button = False

            try:
                next_btn = page.locator(
                    'nav[role="navigation"] a:has-text("Next"), a[aria-label="Next page"], a:has-text(">")'
                )
                if next_btn.count() > 0 and next_btn.first.is_visible():
                    click_like_human(next_btn.first)
                    wait_for_page_settle(page, 2, 4)
                    navigated_via_button = True
            except Exception as e:
                log(f"Next-button navigation failed ({e}); falling back to URL.", "DEBUG")

            if not navigated_via_button:
            ...
            # GYG appends query params like managed_by=...; only the page index must match.
            if reviews_url_matches_page(page.url, next_page_index):
                log(f"On reviews page {next_page_index + 1} ({page.url})", "DEBUG")
            elif reviews_url_matches_page(page.url, previous_page_index):
                log("Still on the same page after navigation. Reached end of pagination.", "INFO")
                break
            else:
                log(
                    f"Unexpected reviews URL after pagination ({page.url}); "
                    f"expected page index {next_page_index}. Stopping.",
                    "WARNING",
                )
                break
